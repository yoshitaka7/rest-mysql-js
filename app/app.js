...
app.get('/api/v1/search', (req, res) => {
  ...
  connection.query(`SELECT * FROM users WHERE name LIKE "%${keyword}%"`, (err, result) => {
    res.json(result)
  })
})
...
app.put('/api/v1/users/:id', async (req, res) => {
  if (!req.body.name || req.body.name === "") {
    ...
    const id = req.params.id
    connection.query(`UPDATE users SET ? WHERE id = ${id}`,req.body,function (err, result, fields) {
      if (err) throw err;
    });
